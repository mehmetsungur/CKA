# Hands-on Kubernetes-25 : Kubernetes Authentication and RBAC
...
- Part 2 - Creating Users in Kubernetes Cluster with csr object

- Part 3 - Creating Users in Kubernetes Cluster Manually

- Part 4 - Using RBAC Authorization
...
- Create `certificate signing request object`. Create a file named `jane-csr.yaml`.
...
metadata:
  name: jane
spec:
  request: "xxxxxxxxxxxxxxxxx" # Get this value with sudo cat /home/jane/jane.csr | base64 | tr -d '\n' command.
...
- Check that the certificate is `approved,issued`.

```bash
kubectl get csr
```

- Get the certificate from csr. It must be base64 decoded.
...
- We send the certificate to jane.

```bash
sudo mv jane.crt /home/jane/
```
...
pwd
#/home/jane
mkdir .certs && mv jane.crt jane.key .certs
```
